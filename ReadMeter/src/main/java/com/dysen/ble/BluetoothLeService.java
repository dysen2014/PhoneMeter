...
public class BluetoothLeService extends Service {
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onCharacteristicChanged(BluetoothGatt gatt,
			...
			System.out.println("Notify:"+new String (characteristic.getValue()));
			broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic);
		}
...
//			intent.putExtra("rx_len", 0);
			intent.putExtra(EXTRA_DATA, String.valueOf(heartRate));
		} else {
			System.out.println("此时的uuid:"+characteristic.getUuid());
...
//				intent.putExtra("rx_len", data.length);
				BleReadMeterActivity.dataLen = data.length;
				if (DeviceControlActivity.flagHex) {
					intent.putExtra(EXTRA_DATA, "\n" + stringBuilder.toString()); // 16进制
				} else {
					intent.putExtra(EXTRA_DATA, "\n" + new String(data)); // 字符串
				}
			}
    ...
    public void writeRXCharacteristic(byte[] value) {
		...
		System.out.println("send byte length:"+value.length);

		int l = value.length / 20;

		if (l != 0){

			for (int i=0; i<l; i++){
				byte[] b = Arrays.copyOfRange(value, i*20, (i+1)*20);
				RxChar.setValue(b);
				status = mBluetoothGatt.writeCharacteristic(RxChar);
				String s = MyUtils.BytesToHexString(b);

				System.out.println("第"+(i+1)+"包发送的数据："+ (s));
				SystemClock.sleep(100);
			}
			if (value.length % 20 != 0) {
				RxChar.setValue(Arrays.copyOfRange(value, l * 20, value.length));
				status = mBluetoothGatt.writeCharacteristic(RxChar);
				System.out.println("第"+(l+1)+"包发送的数据："+ MyUtils.BytesToHexString(Arrays.copyOfRange(value, l * 20, value.length)));
			}
		}else{
			RxChar.setValue(value);
			status = mBluetoothGatt.writeCharacteristic(RxChar);
			System.out.println("发送的数据："+ MyUtils.BytesToHexString(value));
		}
